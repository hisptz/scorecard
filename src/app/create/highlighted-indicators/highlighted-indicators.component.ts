...
import {SetHighlightedIndicator, SetHighlightedIndicatorOu} from '../../store/actions/create.actions';
...
export class HighlightedIndicatorsComponent implements OnInit {
...
// deal with all issues during group type switching between dataelent, indicators and datasets
  ...
  botteneckIndicatorExist(item): boolean {
    let check  = false;
    for ( const indicator of this.highlighted_indicators ) {
  ...
  updateIndicatorDetails(item) {
    this.highlighted_indicators.forEach( (value, index) => {
      if ( value.id === item.id ) {
        this.highlighted_indicators[index] = item;
      }
    });
    this.store.dispatch(new SetHighlightedIndicator([...this.highlighted_indicators]));
  }

  deleteIndicator(item) {
    this.highlighted_indicators.forEach( (value, index) => {
      if ( value.id === item.id ) {
        this.highlighted_indicators.splice(index, 1);
      }
    });
    this.store.dispatch(new SetHighlightedIndicator([...this.highlighted_indicators]));
  }
  ...
  load_item(item): void {
    if (this.botteneckIndicatorExist(item) ) {
      ...
      const indicator = {
        ...
        weight: 100,
        legend_display: true,
        legendset: this.scorecardService.getIndicatorLegendSet(this.scorecard.data.legendset_definitions),
        arrow_settings: {
          effective_gap: 5,
          display: true
        },
      ...
      this.highlighted_indicators.push(indicator);
      this.store.dispatch(new SetHighlightedIndicator([...this.highlighted_indicators]));
    }
  }
