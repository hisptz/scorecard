...
export class BottleneckComponent implements OnInit {
  ...
  loadPriview() {
    if (
      this.indicator.use_bottleneck_groups &&
      this.indicator.bottleneck_indicators_groups.length !== 0
    ) {
      this.prepareChart(this.indicator);
    }
    if (
      !this.indicator.use_bottleneck_groups &&
      this.indicator.bottleneck_indicators.length !== 0
    ) {
      this.prepareChart(this.indicator);
    }
  }
  ...
  addGroup() {
    ...
    this.current_bottleneck_group = this.indicator.bottleneck_indicators_groups[
      this.indicator.bottleneck_indicators_groups.length - 1
    ];
  }
  ...
  deleteGroup(group) {
    const index = _.findIndex(this.indicator.bottleneck_indicators_groups, {
      id: group.id
    });
  ...
  prepareChart(item) {
    ...
    if (labels.length === 0) {
      ...
      this.chartData = this.visulizationService.drawChart(
        this.getSampleAnalytics(labels),
        visualizer_config.chartConfiguration
      );
    }
  }
