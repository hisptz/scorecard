...
export class PeriodFilterComponent implements OnInit {
  ...
  constructor(private httpClient: NgxDhis2HttpClientService) {
    ...
    this.year = this.period_tree_config.starting_year
      ? this.period_tree_config.starting_year
      : date.getFullYear();
    const datestring =
  ...
  deletePeriod(period_to_delete) {
    this.selectedPeriods.forEach((period, period_index) => {
      if (period_to_delete.id === period.id) {
        this.selectedPeriods.splice(period_index, 1);
      }
    });
  }
  ...
  insertPeriod(period_to_insert, current_period, num: number) {
    this.selectedPeriods.forEach((period, period_index) => {
      if (
        current_period.id === period.id &&
        !this.checkPeriodAvailabilty(period_to_insert, this.selectedPeriods)
      ) {
        this.selectedPeriods.splice(period_index + num, 0, period_to_insert);
      }
    });
  }
  ...
  changePeriodType() {
    this.periods = this.getPeriodArray(this.period_type, this.year);
    this.typeUpdate.emit(this.period_type);
  }
