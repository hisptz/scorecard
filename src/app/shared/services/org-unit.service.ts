...
import {OrganisationUnitGroup} from '../models/organisationUnitGroup';
import {OrganisationUnitLevel} from '../models/organisationUnitLevel';
...
export class OrgUnitService {
  ...
  getOrgunitLevelsInformation (): Observable<OrganisationUnitLevel[]> {
    return Observable.create(observer => {
      if (this.orgunit_levels.length !== 0) {
        ...
        this.http.get('organisationUnitLevels.json?fields=id,name,level&order=level:asc')
          .subscribe((levels: any) => {
            this.orgunit_levels = levels.organisationUnitLevels;
            observer.next(this.orgunit_levels);
            observer.complete();
          },
          error => {
  ...
  getOrgunitGroups (): Observable<OrganisationUnitGroup[]> {
    return Observable.create(observer => {
      if (this.orgunit_groups.length !== 0) {
        observer.next(this.orgunit_groups);
        observer.complete();
      }else {
        this.http.get('organisationUnitGroups.json?fields=id,name,organisationUnits[id]&paging=false')
  ...
  // Get system wide settings
  getOrgunits (): Observable<OrganisationUnit[]> {
    return Observable.create(observer => {
      if (this.orgunits.length !== 0) {
        observer.next(this.orgunits);
        observer.complete();
      }else {
        this.http.get('organisationUnits.json?paging=false&fields=id,name,path,level,parent[id],children[id]')
          .subscribe((groups: any) => {
              this.orgunits = groups.organisationUnits;
              observer.next(this.orgunits);
              observer.complete();
            },
            error => {
