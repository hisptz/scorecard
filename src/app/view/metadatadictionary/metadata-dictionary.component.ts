        ...
        .subscribe(data => {
          ...
          if (metadataLink.indexOf('indicators') >= 1) {
            ...
            self.get(indicatorUrl)
              ...
              .subscribe(dataItem => {
                  const indicatorObject = dataItem;
                  self.get('../../../api/expressions/description?expression=' + encodeURIComponent(dataItem.numerator))
                    .subscribe((numExp: Response) => {
                        const numerator = numExp.json().description;
                        self.get('../../../api/expressions/description?expression=' + encodeURIComponent(dataItem.denominator))
                          .subscribe((denoExp: Response) => {
                            const denominator = denoExp.json().description;
                            ...
                            .subscribe(
                              expres => {
                                this.programInd.push({
                                  object: progInd,
                                  filterName: dataItem.description,
